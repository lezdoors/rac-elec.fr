...
// Données authentiques récupérées de la base de données
const clientsData = {
  'RAC-9025-872528': {
    referenceNumber: 'RAC-9025-872528',
    name: 'Bruno PORTES',
    email: 'jbetbp@wanadoo.fr',
    phone: '06 42 06 31 40',
    address: '112 rue des Ecoles, Treves 30750',
    amount: '129,80 €',
    paymentDate: '17 juillet 2025',
    cardInfo: 'Carte bancaire via paiement sécurisé Stripe',
    clientType: 'Particulier'
  },
  'RAC-4515-914201': {
    referenceNumber: 'RAC-4515-914201',
    name: 'Lauralie TERRAL',
    email: 'lauralie.terral@hotmail.fr',
    phone: '06 14 68 13 66',
    address: '2 rue Lucie AUBRAC, Puygouzon 81990',
    amount: '389,40 €',
    paymentDate: '25 juillet 2025',
    cardInfo: 'Carte bancaire via paiement sécurisé Stripe',
    clientType: 'Particulier'
  }
};
        ...
        <div class="section">
            ...
            <table class="info-table">
                ...
                <tr>
                    ...
                    <td class="value">${data.clientType}</td>
                </tr>
            </table>
        </div>
...
async function generateDocuments() {
  try {
    console.log('🚀 Génération des documents DEVOIR DE CONSEIL pour RAC-9025-872528 et RAC-4515-914201...');
    ...
    for (const [ref, data] of Object.entries(clientsData)) {
      ...
      results.push({ reference: ref, filename, url, client: data.name, amount: data.amount });
      
      console.log(`✅ Document créé pour ${ref} - ${data.name}`);
    ...
    const summaryContent = `# NOUVEAUX DOCUMENTS DEVOIR DE CONSEIL ET REÇU - GÉNÉRÉS

## 📄 DOCUMENTS CRÉÉS

### 1. RAC-9025-872528 - Bruno PORTES
- **Client :** Bruno PORTES (Particulier)
- **Email :** jbetbp@wanadoo.fr
- **Téléphone :** 06 42 06 31 40
- **Adresse :** 112 rue des Ecoles, Treves 30750
- **Montant :** 129,80 €
- **Date paiement :** 17 juillet 2025
- **URL :** /certificates/conseil-recu-PDF-RAC-9025-872528.html

### 2. RAC-4515-914201 - Lauralie TERRAL
- **Client :** Lauralie TERRAL (Particulier)
- **Email :** lauralie.terral@hotmail.fr
- **Téléphone :** 06 14 68 13 66
- **Adresse :** 2 rue Lucie AUBRAC, Puygouzon 81990
- **Montant :** 389,40 €
- **Date paiement :** 25 juillet 2025
- **URL :** /certificates/conseil-recu-PDF-RAC-4515-914201.html

## 🔗 LIENS DIRECTS

### RAC-9025-872528
\`/certificates/conseil-recu-PDF-RAC-9025-872528.html\`

### RAC-4515-914201
\`/certificates/conseil-recu-PDF-RAC-4515-914201.html\`

## 📱 TÉLÉCHARGEMENT PDF

1. Cliquer sur le lien
2. Page s'ouvre avec bouton "Télécharger en PDF"
3. Cliquer sur le bouton ou Ctrl+P
4. Choisir "Enregistrer en PDF"
5. Télécharger le fichier PDF

---
*Documents générés le ${new Date().toLocaleDateString('fr-FR')} avec données authentiques*
`;
    
    const summaryPath = path.join(certificatesDir, 'NOUVEAUX-LIENS-TELECHARGEMENT.md');
    await fs.promises.writeFile(summaryPath, summaryContent, 'utf8');
    
    console.log('');
    console.log('🎉 GÉNÉRATION TERMINÉE !');
    console.log('📋 Récapitulatif créé: NOUVEAUX-LIENS-TELECHARGEMENT.md');
    console.log('');
    console.log('🔗 LIENS DIRECTS:');
    results.forEach(result => {
      console.log(`${result.reference} (${result.client} - ${result.amount}): ${result.url}`);
    });
