...
// Générer le PDF en utilisant une approche alternative compatible
async function generatePDFDocuments() {
  try {
    ...
    // Créer le HTML optimisé pour conversion PDF
    const htmlContent = generatePDFOptimizedHTML(clientData);
    
    // Sauvegarder le fichier HTML optimisé
    const htmlPath = path.join(process.cwd(), 'certificates', 'conseil-recu-PDF-RAC-2804-689517.html');
    await fs.promises.writeFile(htmlPath, htmlContent, 'utf8');
    
    console.log('✅ Document HTML optimisé PDF créé');
    console.log('📁 Fichier:', htmlPath);
    console.log('🔗 URL directe:', '/certificates/conseil-recu-PDF-RAC-2804-689517.html');
    
    // Créer un fichier d'instructions pour la conversion
    const instructionsContent = `
# INSTRUCTIONS CONVERSION PDF

## Document généré : conseil-recu-PDF-RAC-2804-689517.html

### Méthode 1 : Conversion navigateur (Recommandée)
1. Ouvrir le lien : /certificates/conseil-recu-PDF-RAC-2804-689517.html
2. Ctrl+P (ou Cmd+P sur Mac)
3. Choisir "Enregistrer en PDF"
4. Format A4, toutes les pages
5. Télécharger le PDF

### Méthode 2 : URL directe
Le document est accessible à l'URL :
https://votre-domaine.com/certificates/conseil-recu-PDF-RAC-2804-689517.html

### Caractéristiques du document
- Format optimisé pour impression A4
- Marges professionnelles
- Police et taille adaptées
- Couleurs compatibles impression
- Toutes les mentions légales incluses

Le document respecte exactement le format des autres certificats (RAC-5706-270576 et RAC-1975-407737).
`;
    
    const instructionsPath = path.join(process.cwd(), 'certificates', 'INSTRUCTIONS-PDF-RAC-2804-689517.txt');
    await fs.promises.writeFile(instructionsPath, instructionsContent, 'utf8');
    
    return {
      htmlUrl: '/certificates/conseil-recu-PDF-RAC-2804-689517.html',
      instructionsUrl: '/certificates/INSTRUCTIONS-PDF-RAC-2804-689517.txt'
    };
...
generatePDFDocuments()
  .then(result => {
    ...
    console.log('📄 Document principal :', result.htmlUrl);
    console.log('📋 Instructions :', result.instructionsUrl);
    console.log('');
    console.log('🔗 LIEN DIRECT DE TÉLÉCHARGEMENT :');
