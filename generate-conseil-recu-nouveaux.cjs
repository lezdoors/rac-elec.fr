...
// Données pour les nouvelles références (à mettre à jour avec les vraies données)
let clientsData = {
  'RAC-9025-872528': {
    referenceNumber: 'RAC-9025-872528',
    name: 'Client à récupérer',
    email: 'email@example.com',
    phone: 'Non renseigné',
    address: 'Adresse à récupérer',
    amount: '259,60 €',
    paymentDate: 'À déterminer',
    cardInfo: 'Carte bancaire via Stripe',
...
// Fonction pour mettre à jour les données avec les vraies informations
function updateClientData(sqlResults) {
    ...
    sqlResults.forEach(row => {
      ...
      if (clientsData[ref]) {
        clientsData[ref] = {
          ...
          phone: row.phone || 'Non renseigné',
          address: row.address ? `${row.address}${row.city ? ', ' + row.city : ''}${row.postal_code ? ' ' + row.postal_code : ''}` : 'Non renseignée',
          amount: row.payment_amount ? `${row.payment_amount} €` : '259,60 €',
          paymentDate: row.payment_date ? new Date(row.payment_date).toLocaleDateString('fr-FR', {
            day: 'numeric',
            month: 'long',
            year: 'numeric'
          }) : 'À déterminer',
          cardInfo: row.card_brand && row.card_last4 ? 
            `Carte ${row.card_brand} •••• ${row.card_last4}${row.card_exp_month && row.card_exp_year ? ` (${String(row.card_exp_month).padStart(2, '0')}/${row.card_exp_year})` : ''}` :
            'Carte bancaire via paiement sécurisé Stripe',
          clientType: row.client_type === 'professionnel' ? 'Professionnel' : 
                      row.client_type === 'collectivite' ? 'Collectivité' : 'Particulier'
        };
      }
    });
  }
...
async function generateDocuments() {
  try {
    ...
    for (const [ref, data] of Object.entries(clientsData)) {
      ...
      results.push({ reference: ref, filename, url, client: data.name });
    ...
    results.forEach(result => {
      console.log(`${result.reference} (${result.client}): ${result.url}`);
    });
...
module.exports = { generateDocuments, updateClientData, clientsData };