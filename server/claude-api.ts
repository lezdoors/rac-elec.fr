...
// Helper function to extract text content from Claude's response
function extractTextFromResponse(response: any): string {
  if (!response || !response.content || !Array.isArray(response.content) || response.content.length === 0) {
    return "Erreur: Réponse inattendue de l'IA";
  }
  ...
  return textBlocks.join('\n') || "Erreur: Aucun contenu texte dans la réponse";
}

export async function analyzeServiceRequest(serviceRequest: ServiceRequest): Promise<string> {
  try {
    const prompt = `
...
export async function generateCustomerResponse(serviceRequest: ServiceRequest): Promise<string> {
  try {
    const prompt = `
...
export async function estimatePrice(serviceRequest: ServiceRequest): Promise<string> {
  try {
    const prompt = `
