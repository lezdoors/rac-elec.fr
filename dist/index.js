...
function setupSmtpService() {
  try {
    ...
    console.log("\u2705 SMTP UNIQUE - notification@raccordement-elec.fr \u2192 bonjour@raccordement-elec.fr");
  } catch (error) {
...
\u{1F3AF} Action requise: Traiter le dossier et contacter le client`
    };
    if (globalTransporter) {
      ...
      <html>
        <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="background: white; padding: 30px; border-radius: 0 0 12px 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.1);">
            ...
            <!-- Informations du paiement \xE9chou\xE9 -->
            ...
            <!-- Informations client \xE0 recontacter -->
...
\u{1F6A8} ACTION URGENTE: Contacter le client dans les 2 heures pour l'accompagner dans le paiement`
    };
    if (globalTransporter) {
      const result = await globalTransporter.sendMail(mailOptions);
...
async function sendLeadNotification(leadData) {
  try {
    ...
    const mailOptions = {
      from: `"Notifications Raccordement" <notification@raccordement-elec.fr>`,
      to: "bonjour@raccordement-elec.fr",
...
async function sendSupportMessageNotification(supportData) {
  try {
    ...
    `;
    const mailOptions = {
      from: `"Support Raccordement" <notification@raccordement-elec.fr>`,
      to: "bonjour@raccordement-elec.fr",
            ...
            <div style="padding: 25px;">
              ...
              <div style="background: #f8f9fa; border: 1px solid #dee2e6; border-radius: 4px; margin-bottom: 20px;">
                ...
                <div style="padding: 20px;">
                  ...
                  <!-- Num\xE9ro de permis -->
                  <div style="margin-bottom: 15px;">
                    <div style="color: #6c757d; font-size: 12px; font-weight: 600; margin-bottom: 4px;">NUM\xC9RO PERMIS DE CONSTRUIRE</div>
                  ...
                  <div style="margin-bottom: 15px;">
                    ...
                    ${requestData.telephoneArchitecte ? `<div style="color: #6c757d; font-size: 14px;">\u{1F4DE} ${requestData.telephoneArchitecte}</div>` : ""}
                    ${requestData.emailArchitecte ? `<div style="color: #6c757d; font-size: 14px;">\u{1F4E7} ${requestData.emailArchitecte}</div>` : ""}
                  </div>
                  ` : ""}
          ...
          <div style="max-width: 600px; margin: 0 auto; background-color: white; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
            ...
            <div style="background: #f1f5f9; padding: 20px; text-align: center; border-top: 1px solid #e2e8f0;">
              <p style="color: #64748b; margin: 0; font-size: 12px;">
                \u{1F4E7} Notification automatique raccordement-elec.fr<br>
                \u23F0 ${(/* @__PURE__ */ new Date()).toLocaleString("fr-FR")}
              </p>
            </div>
      ...
      <html>
        ...
        <body style="margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f8fafc;">
          <div style="max-width: 600px; margin: 0 auto; background-color: white; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
            ...
            <div style="background: #f1f5f9; padding: 20px; text-align: center; border-top: 1px solid #e2e8f0;">
              <p style="color: #64748b; margin: 0; font-size: 12px;">
                \u{1F4E7} Alerte automatique raccordement-elec.fr<br>
...
async function createImapConfigFromUser(userId) {
  try {
    ...
    if (user && user.smtpHost && user.smtpUser && user.smtpPassword && user.smtpEnabled) {
      ...
      if (user.smtpHost === "s4015.fra1.stableserver.net" || emailDomain === "raccordement-elec.fr") {
  ...
  try {
    ...
    try {
      ...
      const emails = await Promise.all(limitedMessages.map(async (message) => {
        ...
        let from = [{ name: "Exp\xE9diteur inconnu", address: "unknown@raccordement-elec.fr" }];
...
<body>
  <div class="receipt">
    ...
    <div class="footer">
      ...
      <p>&copy; ${(/* @__PURE__ */ new Date()).getFullYear()} Raccordement.net - Tous droits r\xE9serv\xE9s</p>
    </div>
  </div>
</body>
</html>`;
...
async function registerRoutes(app2) {
  ...
  console.log("Service SMTP configur\xE9 - notification@raccordement-elec.fr \u2192 bonjour@raccordement-elec.fr");
  ...
  app2.get("/api/admin/notification-email", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      const standardEmails = ["marina.alves@raccordement-elec.fr", "Bonjour@raccordement-elec.fr"];
      ...
      if (notificationEmails.length === 0) {
        notificationEmails = ["notification@raccordement-elec.fr"];
      }
      res.status(200).json({
        success: true,
        data: {
  ...
  app2.post("/api/admin/notification-email", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      const standardEmails = ["marina.alves@raccordement-elec.fr", "contact@raccordement-elec.fr"];
      standardEmails.forEach((email) => {
  ...
  app2.post("/api/leads/create", async (req, res) => {
    try {
      ...
      try {
        ...
        const notificationEmail = notificationEmailConfig?.configValue || "marina.alves@raccordement-elec.fr";
        const emailRecipients = [
          notificationEmail,
          "Bonjour@raccordement-elec.fr",
          "Bonjour@raccordement-elec.fr"
        ];
        const emailData = {
          referenceNumber,
...
${comments}` : tarifJauneNote;
      ...
      try {
        ...
        console.log("\u{1F4EC} Destinataire principal: bonjour@raccordement-elec.fr");
            ...
            case "succeeded":
              ...
              if (serviceRequest.paymentStatus !== "paid") {
                ...
                try {
                  ...
                  const isRaccordementElecDomain = hostHeader.includes("raccordement-elec.fr") || refererHeader.includes("raccordement-elec.fr") || hostHeader.includes("replit.dev");
                  ...
                  console.error("\u274C Erreur notification paiement raccordement-elec.fr:", emailError);
                }
                await storage.updateServiceRequestStatus(
                  serviceRequest.id,
                  REQUEST_STATUS.PAID,
            ...
            case "requires_payment_method":
              ...
              if (serviceRequest.paymentStatus !== "failed") {
                ...
                try {
                  ...
                  console.error("\u274C Erreur lors de l'envoi de la notification de paiement \xE9chou\xE9 raccordement-elec.fr:", emailError);
                }
              }
              break;
  ...
  app2.get("/api/system-config", requireAuth, requireAdmin, async (req, res) => {
    try {
      const configs = {
        ...
        email: {
          provider: "sendgrid",
          senderEmail: "contact@raccordement-elec.fr",
      ...
      <body>
        ...
        minute: "2-digit"
          ...
          <div class="footer">
            ...
            <p>Pour toute question, veuillez nous contacter \xE0 contact@raccordement-elec.fr</p>
  ...
  if (process.env.NODE_ENV === "development") {
    ...
    app2.post("/api/test/send-email", async (req, res) => {
      try {
        ...
        const recipients = notificationEmailRow ? notificationEmailRow.configValue.split(",") : ["contact@raccordement-elec.fr"];
        const referenceNumber = `REF-${Math.floor(1e3 + Math.random() * 9e3)}-${Math.floor(1e5 + Math.random() * 9e5)}`;
        const info = await transporter.sendMail({
          from: `"Service Raccordement" <${process.env.SMTP_USER || "notification@raccordement-elec.fr"}>`,
          to: recipients.join(", "),
